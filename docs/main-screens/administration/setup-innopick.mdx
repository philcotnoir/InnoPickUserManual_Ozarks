...
The Setup InnoPick page contains configuration settings for InnoPick's internal operation. Access is typically restricted to the Admin and Technical Support roles.
...
Limits consecutive cases sent to a single destination (Spiral A, B, or C).
...
### Number Of Cases To Exit Spiral In The Accumulation LookAhead
How many cases IPM considers when forecasting and scheduling output. This is only applicable if Destination Decoupling is Disabled. This # is meant to reprensent how many cases can exit the Spiral during the time it takes a case to travel the entire length of InnoPick.
...
### Destination A / B / C Accumulation
...
- This setting teaches InnoPick how many cases can fit between the exit of the Spiral and the palletizer.

**Level 1 to 6: Accumulation Length in Mm**
...
**Level 1 to 6: Zone Accumulation (# of zones)**

- This setting teaches InnoPick how many individual cases can be accumulated between the exit of InnoPick and the Spiral.
